...
const StatusBadge = ({ status = "", size = "default", showIcon = true }) => {
  ...
  return (
    <div
     ...
     ${config.bg} ${config.text} ${config.border} transition-all duration-300 ease-out hover:scale-105 hover:shadow-sm group relative
     overflow-hidden`}
    >
      <div
        className={`absolute inset-0 bg-linear-to-r ${config.gradient} opacity-0 group-hover:opacity-5
    ...
    return (
        <div className="inline-flex items-center gap-2 group">
            ...
            {count !== undefined && (
              ...
              px-2 py-1 rounded-full group-hover:bg-gray-200 transition-colors duration-200">{count}</span>  
            )}
        </div>
    )
}
