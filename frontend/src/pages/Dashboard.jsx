...
function currencyFmt(amount = 0, currency = "NGN") {
  try {
...
const Dashboard = () => {
  ...
  // to obtain the token and check if the user is signed in
  ...
  //NOTE: fetch invoices from backend
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        //NOTE unauthorized - prompt login
      ...
      const mapped = (Array.isArray(raw) ? raw : []).map((inv) => {
        ...
        const currency = (inv.currency || "NGN").toUpperCase();
  ...
  //NOTE: currency conversion, 1 usd = 1200 naira
  const HARD_RATES = {
    USD_TO_NGN: 1200,
  };

  function convertToNGN(amount = 0, currency = "NGN") {
    const n = Number(amount || 0);
    const curr = String(currency || "NGN")
      .trim()
      .toUpperCase();

    if (curr === "NGN") return n;
    if (curr === "USD") return n * HARD_RATES.USD_TO_NGN;
    return n;
  }
  //NOTE: KPIs computation with percentages i.e(convert each invoices to naira)
  const kpis = useMemo(() => {
    const totalInvoices = storedInvoices.length;
    let totalPaid = 0; // in naira
    let totalUnpaid = 0; // in naira
    let paidCount = 0;
    let unpaidCount = 0;
    ...
    storedInvoices.forEach((inv) => {
      ...
      const amtInINR = convertToNGN(rawAmount, invCurrency);
  ...
  //NOTE: recent invoices by created at date in database.
  ...
  //NOTE: get client name from invoice
  ...
  //NOTE: get client initial from invoice
    ...
    <div className={dashboardStyles.pageContainer}>
      <div className={dashboardStyles.headerContainer}>
        ...
        <p className={dashboardStyles.headerSubtitle}>
          Get real-time insights into your invoicing and business performance
        </p>
      </div>

      {/* //NOTE - Loading error state */}
      {/* {loading ? (
      ...
      <div className="p-6">
        ...
        <div className="flex gap-2">
          <button onClick={fetchInvoices} className="px-3 py-1 bg-blue-600 text-white
          rounded">
            Retry
          </button>
          {String(error).toLowerCase().includes("unauthorized") && (
            <button onClick={() => navigate("/login")} className="px-3
            py-1 bg-gray-700 text-white rounded">
              Sign in
            </button>
          )}
        </div>
      ...
      <div className={dashboardStyles.kpiGrid}>
        ...
        <KpiCard
          title="Total Paid"
          value={currencyFmt(kpis.totalPaid, "NGN")}
          hint="Received Amount (NGN)"
          iconType="revenue"
          trend={0}
        />
        <KpiCard
          title="Total Unpaid"
          value={currencyFmt(kpis.totalUnpaid, "NGN")}
          hint="Outstanding balance (NGN)"
          iconType="clock"
          trend={-0}
        />
      </div>
